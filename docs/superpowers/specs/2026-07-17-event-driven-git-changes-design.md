...
- Both the regular file viewer and Git diffs wrap source at the viewport edge; neither view has horizontal scrolling. The file viewer renders original source lines separately and measures the requested line's rendered Y offset before scrolling, so terminal links still land on their exact source line despite variable wrapped heights.
