...
describe('noiseSessionAddress', () => {
  it('dials wss on the TLS ports', () => {
    ...
    expect(noiseSessionAddress(profile('8443'))).toBe('wss://tether.example:8443/api/noise/session');
  });
  ...
  it('honours a recorded https scheme on a non-standard port', () => {
    expect(noiseSessionAddress(profile('9999', 'https'))).toBe('wss://tether.example:9999/api/noise/session');
  });
});
