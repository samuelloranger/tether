...
`App.tsx` is 741 lines with ~620 inside `App()` and 16 hooks;
`src-tauri/src/commands/noise.rs` is 1217. Both are split candidates, both are
code changes rather than moves, so each gets its own PR.
