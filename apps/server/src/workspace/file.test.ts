...
import { canonicalPath, inside, readWorkspaceFile, samePath, WorkspaceFileError } from './file';
...
test('inside() is case-sensitive, and never loosens containment', () => {
  const root = path.resolve('/srv/Workspace');
  const child = path.join(root.toUpperCase(), 'file.txt');
  // /srv/Workspace and /SRV/WORKSPACE are genuinely different directories here.
  // Folding case would let one be mistaken for the other, which is a real
  // weakening rather than a portability fix.
...
test('canonicalPath is idempotent, so containment compares like with like', () => {
  withRoot((root) => {
    mkdirSync(path.join(root, 'src'));
...
test('a root and a cwd spelled differently still describe one workspace', () => {
  withRoot((root) => {
    ...
    const shortishCwd = path.join(root, 'src');
