...
}
{
  ...
  eq(inv.args[0], 'bash', 'bash: basename-resolved from a full path');
}

// zsh: ZDOTDIR redirect to our injected .zshrc (sources the user's real
// ~/.zshrc, then hooks OSC 7 via precmd_functions — invisible, no prompt
// replacement since zsh users already have their own).
{
...
}
{
  ...
  eq(inv.args[0], 'fish', 'fish: basename-resolved from a full path');
}
