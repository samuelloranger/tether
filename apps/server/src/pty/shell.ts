...
export type ShellIntegration = 'full' | 'none';

export interface ShellSupport {
  /** The shell name the classification was made on ('bash', 'zsh'…). */
  shell: string;
...
/** The shells shellInvocation() has an integration hook for. */
const INTEGRATED = new Set(['bash', 'zsh', 'fish']);

/** Classifies a shell command, without running it or touching the filesystem. */
export function describeShellSupport(command: string): ShellSupport {
  const shell = path.posix.basename(command);
