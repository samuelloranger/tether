...
// longer exists — passing its stale cwd back as the answer. The random suffix
// covers the rest of it: a pid alone is recycled between runs.
...
// The fs.watch-driven tests opt the watcher in explicitly rather than relying on
// the default, so a change to that default cannot silently stop exercising it.
