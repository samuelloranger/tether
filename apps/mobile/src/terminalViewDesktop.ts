...
function attachWebgl(terminal: Terminal, callbacks: DesktopTerminalCallbacks): WebglAddon | null {
  try {
    const webgl = new WebglAddon();
    webgl.onContextLoss(() => {
      webgl.dispose();
      callbacks.onFallback('webgl-context-lost');
      terminal.refresh(0, terminal.rows - 1);
    });
    terminal.loadAddon(webgl);
    return webgl;
  } catch (error) {
    callbacks.onFallback(String(error));
    return null;
  }
}
...
export function mountDesktopTerminal(opts: MountDesktopOpts): () => void {
  ...
  const webgl = attachWebgl(terminal, opts.callbacks.current);
