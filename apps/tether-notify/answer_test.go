...
func answerArgs(since string, input string, extra ...string) []string {
	return append([]string{"--session", "work", "--state", "waiting", "--since", since,
		"--input", base64.StdEncoding.EncodeToString([]byte(input))}, extra...)
}

var waiting = &SessionState{Session: "work", Agent: "claude", State: stateWaiting, Since: 1000, Updated: 1000}

func TestAnswerTypesTheKeysWhileThePromptIsCurrent(t *testing.T) {
	d, sends, _ := answerFixture(t, waiting)
	if err := runAnswer(answerArgs("1000", "\r"), d); err != nil {
		t.Fatal(err)
	}
...
func TestAnswerSubmitsAReplyInASecondWrite(t *testing.T) {
	...
	if err := runAnswer(answerArgs("1000", "run the tests; rm -rf / $(x)", "--submit"), d); err != nil {
		t.Fatal(err)
	}
	want := []sendCall{
...
func TestAnswerRefusesAStalePrompt(t *testing.T) {
	cases := map[string]*SessionState{
		"newer prompt":  {Session: "work", State: stateWaiting, Since: 2000},
		"agent working": {Session: "work", State: stateWorking, Since: 1000},
		"no session":    nil,
	}
	for name, stored := range cases {
		d, sends, _ := answerFixture(t, stored)
		if err := runAnswer(answerArgs("1000", "\r"), d); !errors.Is(err, errStale) {
...
func TestAnswerRejectsFlagLikeSessionsAndBadInput(t *testing.T) {
	d, sends, _ := answerFixture(t, waiting)
	bad := [][]string{
		{"--session", "--help", "--state", "waiting", "--since", "1000", "--input", "DQ=="},
		{"--session", "-h", "--state", "waiting", "--since", "1000", "--input", "DQ=="},
		{"--session", "work", "--state", "waiting", "--since", "1000", "--input", "!!"},
		{"--session", "work", "--state", "waiting", "--since", "1000", "--input", ""},
		{"--session", "work", "--state", "bogus", "--since", "1000", "--input", "DQ=="},
		{"--session", "work", "--state", "waiting", "--since", "0", "--input", "DQ=="},
	}
